...
public class GFCBaseController extends GenericForwardComposer<Component> implements Serializable {
	...
	private final String longDateTimeFormat		= "dd-MMMM-yyyy HH:mm:ss";
	private final String shortDateFormat 		= "dd-MM-yyyy";
	private final String shortDayDateFormat		= "EEE dd-MM-yyyy";
	private final String shortDateTimeFormat	= "dd-MM-yyyy HH:mm:ss";
	...
	private final String shortTimeFormat	= "HH:mm";
	 ...
	 * @return {@link Double}
	 */
	public double toDecimalHours(int hours, int minutes) {
        ...
        double decimalMinutes = (double) minutes / 60.0;
        return hours + decimalMinutes;
    }
	
	public double roundToDecimalPlaces(int scale, double value) {
	 ...
	 * format must be in the form of: dd-MMMM-yyyy to produce the string value.
	...
	/**
	 * datetimeToStringDisplay returns the date and time with a specified Locale,
	 * to a string value according to the formatStr parameter.  The
	 * format must be in the form of: dd-MMMM-yyyy HH:mm to produce the string value.
	 * Locale is required for the month's spelling.
	 * 
	 * @param localDateTime
	 * @param format
	 * @param locale
	 * @return {@link String}
	 */
	public String datetimeToStringDisplay(LocalDateTime localDateTime, String format, Locale locale) {
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern(format, locale);
		
		return localDateTime.format(formatter);
	}
	
	public String timeToStringDisplay(LocalTime localTime, String format, Locale locale) {
		DateTimeFormatter formatter = DateTimeFormatter.ofPattern(format, locale);
	...
	public String toDecimalFormat(BigDecimal nominal, Locale locale, String decimalFormat) throws Exception {
		BigDecimal nominalRound = nominal.setScale(2, RoundingMode.HALF_UP);
...
//		// System.out.println(userSecurityDetails.getUser().toString());
//		
